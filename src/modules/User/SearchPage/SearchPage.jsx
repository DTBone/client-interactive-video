...
} from "~/store/slices/SearchCourseForUser/searchCourseAPI";
...
const SearchPage = () => {
  ...
  const [hasMore, setHasMore] = useState(
    useSelector((state) => state.search.hasMore)
  );
  const loading = useSelector((state) => state.search.loading);

  const [recentFilters, setRecentFilters] = useState([]);

  const { query, filters, currentPage, isFilterApplied, totalPages } =
    searchState;
  const [searchParams, setSearchParams] = useSearchParams();
  const [
    searchCourses,
    { data, isLoading: searchLoading, error: searchError },
  ] = useLazySearchCoursesQuery();
  ...
  const applyFiltersAutomatic = useCallback(
    (tags, levels, price, ratingValue, sort) => {
  ...
  const saveRecentFilter = (tags, levels, price, ratingValue, sort) => {
    ...
    if (
      ...
      const exists = recentFilters.some(
        (filter) =>
  ...
  const fetchMoreData = () => {
    ...
    try {
      ...
      if (isAnyFilterApplied()) {
        ...
        console.log(
          `Debug pagination - Current page: ${currentPage}, Total pages: ${totalPagesFromResponse}, Has next page: ${hasNextPage}`
        );
        ...
        const canLoadMore =
          ...
          (totalPagesFromResponse > 0 &&
            currentPage < totalPagesFromResponse) ||
        ...
        if (canLoadMore) {
          ...
          console.log(
            `Đang tải thêm dữ liệu trang ${nextPage}/${totalPagesFromResponse || "unknown"}`
          );
          searchCourses(searchObj);
        } else {
        ...
        console.log(
          `Debug No Filter - Current page: ${page}, Total pages: ${totalPages}`
        );

        // Kiểm tra nếu có totalPages trả về và còn trang để tải
        if (totalPages > 0 && page < totalPages) {
          console.log(
            `Đang tải thêm tất cả khóa học: trang ${page + 1}/${totalPages}`
          );
        ...
        // Nếu không có thông tin totalPages rõ ràng, dùng kiểm tra dựa trên số lượng kết quả
          ...
          const hasMoreByCount =
            courses && courses.length > 0 && courses.length % pageLimit === 0;

          if (hasMoreByCount) {
            console.log(
              `Đang tải thêm tất cả khóa học (dựa trên số lượng): trang ${page + 1}`
            );
            dispatch(fetchCourses({ page: page + 1, limit: pageLimit }));
          } else {
            console.log(
              "Đã tải hết tất cả khóa học (dựa trên số lượng kết quả)"
            );
            setHasMore(false);
          }
        }
  ...
  useEffect(() => {
    if (data && data.data) {
      ...
      if (pagination.hasNextPage !== undefined) {
        ...
        console.log(
          `Có thể tải thêm (dựa vào hasNextPage): ${pagination.hasNextPage}`
        );
  ...
  const saveFilterToHistory = () => {
    ...
    if (
      ...
      const filterExists = filterHistory.some(
        (filter) =>
          JSON.stringify({ ...filter, timestamp: undefined }) ===
          JSON.stringify({ ...currentFilter, timestamp: undefined })
      );
      ...
      <section className="w-full lg:w-3/4 bg-white rounded-lg shadow-sm p-4">
        ...
        {searchError && (
          <div className="text-red-500 p-4 border border-red-300 rounded">
            Error:{" "}
            {searchError.message || "An error occurred while loading data."}
          </div>
        )}
        ...
        <div className="overflow-hidden">
          {!loading && !searchLoading && displayedCourses?.length === 0 ? (
            <div className="flex justify-center items-center h-64">
              <p className="text-gray-500 text-lg">No suitable course found</p>
            </div>
          ) : (
            <InfiniteScroll
