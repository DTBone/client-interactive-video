...
import { Download, FileText, BookOpen, Maximize, ZoomIn, ZoomOut, ChevronLeft, ChevronRight, Loader } from 'lucide-react';
...
const Supplement = () => {
    ...
    const handleCompeleSupplement = async () => {
        const rep = await dispatch(updateSupplementProgress({ 
            progressId: progress._id, 
            progressSupplement: { status: 'completed', supplementId: item._id } 
        }));
    ...
    useEffect(() => {
        if (progress && 
            ...
            progress.moduleItemProgresses.find(p => p.moduleItemId === item._id)?.status === 'completed') {
            return;
        }
    ...
    if (!item) return (
        <div className="flex flex-col items-center justify-center h-[400px] text-gray-500">
            ...
            <p className="text-sm mt-2">Please select another lesson with supplementary materials</p>
        </div>
    );
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between bg-gray-800 text-white p-3">
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <button onClick={handleFullscreen} className="p-1 hover:bg-gray-700 rounded ml-2">
                                    <Maximize size={18} />
                                </button>
                            </div>
                        </div>
