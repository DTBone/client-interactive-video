...
const AnswerContainer = styled(Box)(({ theme, correct }) => ({
    ...
    backgroundColor: correct ? alpha(theme.palette.success.light, 0.1) : alpha(theme.palette.background.paper, 0.5),
    border: `1px solid ${correct ? alpha(theme.palette.success.main, 0.5) : theme.palette.divider}`,
    borderRadius: theme.spacing(1),
    marginBottom: theme.spacing(1.5),
    transition: 'all 0.2s ease',
    '&:hover': {
        backgroundColor: correct ? alpha(theme.palette.success.light, 0.15) : alpha(theme.palette.primary.light, 0.05),
...
const EditQuiz = ({ moduleItem }) => {
    ...
    const getFormCompletionPercentage = () => {
        ...
        const hasCompleteQuestion = quizData.questions.some(q => 
            q.content.trim() && q.answers.some(a => a.content.trim())
        );
        ...
        if (moduleItem) {
            try {
                setQuizData(prev => ({
                    ...
                    contentType: moduleItem.contentType || 'Practice Quiz',
                    icon: moduleItem.icon || 'quiz',
                    duration: moduleItem.quiz?.duration || 1200,
                    passingScore: moduleItem.quiz?.passingScore || 70,
                    isGrade: moduleItem.isGrade || false,
                ...
                showNotice('error', 'Failed to load quiz data');
            } finally {
                setIsLoading(false);
            }
        }
    }, [moduleItem, showNotice]);
    ...
    const addQuestion = () => {
        ...
        setTimeout(() => {
            document.getElementById(`question-${quizData.questions.length}`)?.scrollIntoView({ 
                behavior: 'smooth', 
                block: 'center' 
            });
        }, 100);
    ...
    const removeQuestion = (index) => {
        ...
        if (quizData.questions[index].content.trim() && 
    ...
    const addAnswer = (questionIndex) => {
        ...
        setTimeout(() => {
            ...
            const inputElement = document.getElementById(`answer-${questionIndex}-${answerCount}`);
            if (inputElement) {
                inputElement.focus();
            }
        }, 100);
    ...
    const removeAnswer = (questionIndex, answerIndex) => {
        ...
        const isRemovingOnlyCorrectAnswer = 
            ...
            quizData.questions[questionIndex].answers.filter(a => a.isCorrect).length === 1;
            
        if (isRemovingOnlyCorrectAnswer) {
            if (!window.confirm("You are removing the only correct answer. Another answer will be automatically marked as correct. Continue?")) {
                return;
            }
        }
    ...
    const handleAnswerChange = (questionIndex, answerIndex, field) => (event) => {
        ...
        const updatedQuestions = quizData.questions.map((question, qIndex) => {
            if (qIndex === questionIndex) {
                ...
                // Special handling for "only-choice" questions - only one answer can be correct
                if (field === 'isCorrect' && value === true && question.type === 'only-choice') {
                    ...
                    if (correctAnswersCount <= 1 && question.answers[answerIndex].isCorrect) {
                        showNotice('warning', 'At least one answer must be marked as correct');
    ...
    const handleGradeChange = (event) => {
        ...
        setHasChanges(true);
        showNotice('info', event.target.checked ? 'This quiz will be graded' : 'This quiz will not be graded');
    };
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (!validateForm()) {
                showNotice('error', 'Please fix the errors in the form before submitting');
                setIsSubmitting(false);
                return;
            }
            
            // Submit the quiz data
            const resultAction = await dispatch(editQuizByItemId({
            ...
            if (!resultAction.error) {
                ...
                throw new Error(resultAction.error.message || 'Failed to update quiz');
            }
        } catch (error) {
            console.error('Error submitting quiz:', error);
            showNotice('error', error.message || 'Error updating quiz');
        } finally {
            setIsSubmitting(false);
        }
    };
                ...
                <CardContent sx={{ p: 3 }}>
                    ...
                    {isLoading && (
                        <Box sx={{ display: 'flex', justifyContent: 'center', my: 4 }}>
                            <CircularProgress />
                            <Typography sx={{ ml: 2 }}>Loading quiz data...</Typography>
                        </Box>
                    )}
                    ...
                    {hasChanges && !isLoading && (
                        <Alert 
                            ...
                            icon={<EditIcon />}
                        >
                            You've made changes to this quiz. Don't forget to save your changes!
                        </Alert>
                    )}
                        ...
                        <Box component="form" noValidate sx={{ mt: 2 }}>
                                ...
                                <CardContent>
                                    <Grid container spacing={3}>
                                        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            ...
                                            <Tooltip title="If enabled, quiz results will count towards the final grade" arrow>
                                                <FormControlLabel
                                                    control={
                                                        <Switch
                                                    ...
                                                    label={
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                            <Star fontSize="small" color={quizData.isGrade ? "primary" : "disabled"} />
                                        ...
                                        <Grid item xs={12}>
                                            <StyledTextField
                                                ...
                                                helperText={formErrors.title || "Enter a clear, descriptive title for your quiz"}
                                                InputProps={{
                                                    startAdornment: (
                                        ...
                                        <Grid item xs={12}>
                                            <StyledTextField
                                                ...
                                                helperText={formErrors.description || "Provide instructions and information about the quiz purpose"}
                                                InputProps={{
                                                    startAdornment: (
                                                        <InputAdornment position="start">
                                        ...
                                        <Grid item xs={12} sm={6}>
                                            <StyledTextField
                                                ...
                                                onChange={(e) => handleQuizChange('duration')({ 
                            ...
                            <Box sx={{ mb: 3 }}>
                                ...
                                {quizData?.questions.map((question, questionIndex) => (
                                    <Zoom in={true} key={questionIndex} style={{ transitionDelay: `${questionIndex * 100}ms` }}>
                                        <QuestionCard 
                                            ...
                                            sx={{
                                                mb: 3,
                                                transition: 'all 0.2s',
                                                transform: activeQuestion === questionIndex ? 'translateY(-4px)' : 'none',
                                                boxShadow: activeQuestion === questionIndex ? '0 8px 24px rgba(0, 0, 0, 0.2)' : undefined,
                                            }}
                                        >
                                            <QuestionNumber 
                                                ...
                                                <Box sx={{ 
                                                    ...
                                                    <Typography 
                                                        ...
                                                        sx={{ 
                                                            display: 'flex', 
                                                            alignItems: 'center',
                                                            color: question.content ? 'text.primary' : 'text.disabled'
                                                        }}
                                                    >
                                                        {question.content 
                                                            ? question.content.substring(0, 40) + (question.content.length > 40 ? '...' : '')
                                                            : 'New Question'
                                                        }
                                                    </Typography>
                                                    ...
                                                    <Tooltip title="Remove Question" arrow>
                                                        <span>
                                                            <IconButton
                                                                ...
                                                                disabled={quizData.questions.length === 1 || isSubmitting}
                                                                size="small"
                                                            >
                                                                <DeleteIcon />
                                                            </IconButton>
                                                ...
                                                <Grid container spacing={2}>
                                                    <Grid item xs={12}>
                                                        <StyledTextField
                                                            ...
                                                            error={!!formErrors.questions[questionIndex]?.content}
                                                            helperText={formErrors.questions[questionIndex]?.content || ''}
                                                            multiline
                                                            rows={2}
                                                        />
                                                    </Grid>
                                                    ...
                                                    <Grid item xs={12}>
                                                        <Box sx={{ mt: 2, mb: 1 }}>
                                                            ...
                                                            {formErrors.questions[questionIndex]?.answerContent && (
                                                                <Typography color="error" variant="caption" sx={{ display: 'block', mt: 0.5 }}>
                                                                    {formErrors.questions[questionIndex].answerContent}
                                                                </Typography>
                                                            )}
                                                    ...
                                                    <Grid item xs={12}>
                                                        <StyledTextField
                                                            ...
                                                            InputProps={{
                                                                startAdornment: (
                                                                    <InputAdornment position="start">
                                                                        <Help color="primary" sx={{ alignSelf: 'flex-start', mt: 1 }} />
                                                                    </InputAdornment>
                                                                ),
                                                            }}
