...
const EditProgramming = ({ moduleItem }) => {
  ...
  useEffect(() => {
    if (moduleItem) {
      // Đảm bảo mỗi code format có language, fallback to python nếu không có
      const processedCodeFormat = moduleItem.programming.codeFormat.map(
        (code) => ({
          ...code,
      ...
      setProblemData({
        ...
        problemName: moduleItem.programming.problemName
          ? moduleItem.programming.problemName
          : "",
        ...
        codeFormat: processedCodeFormat,
      ...
      console.log(
        "ModuleItem loaded with processed codeFormat:",
        processedCodeFormat
      );
    }
  }, [moduleItem]);
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmAlert
        ...
        title={
          showConfirm.type === "code" ? "Delete Code Sample" : "Delete Testcase"
        }
          ...
          {problemData.codeFormat.map((code, index) => (
            <div key={index} className="mb-6 p-4 border rounded bg-gray-50">
              <div className="flex justify-between items-center mb-4">
                ...
                <IconButton
                  onClick={() =>
                    setShowConfirm({ type: "code", index, open: true })
                  }
          ...
          {problemData.testcases.map((testcase, index) => (
            <div key={index} className="mb-6 p-4 border rounded bg-gray-50">
              <div className="flex justify-between items-center mb-4">
                ...
                <IconButton
                  onClick={() =>
                    setShowConfirm({ type: "testcase", index, open: true })
                  }
